...
This quickstart takes you from zero to a real result you can verify: a list of your idle EC2 instances with projected monthly savings, returned by an AI agent reading your actual AWS account. Allow **~10 minutes**, mostly spent creating the AWS role and waiting for CloudThinker to inventory your resources.
...
<Steps>
  <Step title="Register">
    ...
    **Success state:** You land in your personal organization with a default [workspace](/guide/workspaces) — the shared area that holds your connections, chats, and agents — ready. The left sidebar shows **New chat**, **Skills**, and **Artifacts** at the top, and an empty conversation panel on the right.
  ...
  <Step title="Connect AWS">
    <Tip>
    No AWS account handy, or not ready to hand over credentials? Try the read-only [demo environment](/guide/connections/overview#try-the-demo-environment) first: pick a connection marked **Demo available**, take the demo offer in the connect dialog, and you land in a chat seeded with an editable overview prompt. Demo chats spend credits like real ones. Come back here when you're ready to connect your own account.
    </Tip>
    ...
    **Success state:** the connection card flips to **Connected** with a green dot. Discovery — CloudThinker's automatic inventory of your cloud resources — starts immediately; within ~2 minutes you'll see a non-zero resource count (instances, RDS, S3, etc.) on the card. If discovery stays at zero, jump to [Troubleshooting](#troubleshooting) below.
  </Step>

  <Step title="Verify discovery">
    Return to **Settings → Connections** and open the AWS connection card.

    You should see:

    - A list of regions with non-zero resource counts
    - At least one of: EC2 instances, S3 buckets, RDS databases, Lambda functions
    - A successful discovery status without `AccessDenied` errors
  ...
  <Step title="Run your first prompt">
    ...
    [Alex](/guide/agents/alex), the cloud engineer agent, queries CloudWatch utilization metrics, joins them with the resource inventory, and returns a structured summary.

    **Success state:** a response within ~30 seconds containing:
...
- [x] You can see your AWS resource count on the connection card and it matches reality
...
From your next sign-in, the [Home screen](/guide/activity) is your starting point: a **Needs you** feed of anything waiting on your approval, a status line per module, and a banner for any live incident.
...
<AccordionGroup>
  ...
  <Accordion title="Slow response or timeout on the first prompt" icon="hourglass-half">
    ...
    **Fix:** wait until the AWS connection card shows non-zero resource counts, then re-run the prompt. Discovery typically completes in 2–5 minutes for an account with under 500 resources.
  </Accordion>
</AccordionGroup>
