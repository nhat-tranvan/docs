...
<Steps>
  ...
  <Step title="Grant change access (optional)">
    ...
    ```sql
    GRANT MODIFY ON KEYSPACE your_keyspace TO ct_agent;  -- single-row insert, update, delete
    GRANT ALTER  ON KEYSPACE your_keyspace TO ct_agent;  -- table TTL, gc_grace, secondary indexes
    ```
  </Step>
...
| **Role name** | A dedicated login role, for example `ct_agent` | — |
...
- A role created without `WITH LOGIN = true` authenticates nowhere. Recreate it or `ALTER ROLE ct_agent WITH LOGIN = true;`
</Accordion>
