  ...
  <Tab title="Role-Based (Recommended)">
    ...
    <Steps>
      ...
      <Step title="Complete the connection">
        Paste the Role ARN into CloudThinker, optionally select a default region, and click **Connect**. CloudThinker verifies the role and shows a **Connected** status.
      </Step>
    </Steps>
...
| **Default AWS region (optional)** | Starting region for this connection. Agents can use other regions allowed by your permissions and explicit scope restrictions. | `us-east-1` |
...
```text
@alex #recommend right-sizing opportunities from EC2 costs over the last 30 days
@oliver #report IAM roles with excessive permissions or missing MFA requirements
@kai #report EKS cluster health and any failing pods across all connected regions
```
...
<Accordion title="Connection shows Error and scheduled work stopped">
Three consecutive credential failures in background runs — a broken trust policy, a deleted role, or a revoked access key — switch the connection to **Error**, and every schedule that depends on it pauses at once: [Optimize](/guide/cost-optimization/overview) cost refreshes, scheduled [automations](/guide/automation/automations), and [Pulse](/guide/pulse/overview) alert polling. Workspace admins are notified once per outage.

A permission gap on a single feature — missing Cost Explorer permissions, for example — never disables the whole connection; it only limits that feature.

**Fix:** repair the role or key in AWS, then re-test the connection in CloudThinker. When the status returns to **Connected**, schedules resume on their next cycle.
</Accordion>
