...
<Accordion title="Savings plan or reserved instance coverage looks empty">
Grant `ce:GetSavingsPlans*`, `ce:GetReservation*`, and `savingsplans:DescribeSavingsPlans`. AWS returns nothing rather than an error when these are denied, so a role without them reports commitment coverage as if the account had nothing to fix.

Test the connection again after updating the policy. The result names each denied action, and the CostOps scan history shows `permission needed:` followed by the same list.
</Accordion>
