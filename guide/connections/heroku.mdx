...
<Warning>
Copy the token immediately after creation. Heroku does not show it again, and you will need to create a new authorization if it is lost.
</Warning>
...
Private Spaces are a Heroku Enterprise feature. An account without them returns an empty list, which is expected rather than an error.
</Accordion>
...
- **Revoke rather than delete** — `heroku authorizations:revoke <id>` blocks the token immediately, and the Heroku Dashboard lists account authorizations under **Account → Applications** for review.
