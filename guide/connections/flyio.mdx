...
|-------|-------------|---------|
| **Access token** | Read-only Fly.io organization token for the organization CloudThinker should inspect | — |
...
The token type *is* the permission model. `fly tokens create readonly` produces a token limited to reading a single organization and its resources — it cannot deploy, scale, restart, or destroy anything, and no additional scoping is needed or available. Keep the read-only token type, set a short `--expiry`, and revoke the token when the work it was created for is finished.
...
App `status` and machine `state` are Fly.io control-plane records, not application health checks. Four consequences are worth knowing before you act on a report:
...
```text
@alex #report my Fly.io apps with their status and deployment state as a table
@alex #recommend what to check next for my Fly.io app based on its machine states and regions
@alex #report unattached Fly.io volumes and total provisioned GB for my app
```
...
- **Read-only at the provider** — the token type Fly.io issues here cannot make changes, so the guarantee holds even if something asks it to, and a leaked token's blast radius stays one organization.
- **Set a short expiry and revoke when finished** — Fly.io defaults tokens to 20 years, so always pass `--expiry`; `fly tokens revoke <token id>` invalidates one, and the Fly.io dashboard lists organization tokens under **Tokens**.
