---
title: PostHog
description: "Connect PostHog to CloudThinker for product analytics, error triage, HogQL queries, and approval-gated flag and dashboard changes"
icon: "/images/icons/posthog.svg"
---

import ConnectionSecurity from '/snippets/connection-security.mdx';

Connect your PostHog project to let [Alex](/guide/agents/alex) (Cloud Engineer) read what your product is doing — feature flags, insights, dashboards, surveys, tracked errors, and your event schema — and answer questions with bounded queries in HogQL, PostHog's SQL-like query language.
...
<Steps>
  ...
  <Step title="Connect in CloudThinker">
    Go to **Connections → PostHog**, paste the key into **POSTHOG_PERSONAL_API_KEY**, select your **POSTHOG_REGION**, then click **Connect**. CloudThinker verifies the key and shows a **Connected** status.
  </Step>
</Steps>
...
```text
@alex #report my PostHog setup: organization, active project, and whether feature flags and insights exist
```

### Example prompts

```text
@alex #report which errors are hurting users most right now
@alex #visualize signup event volume over the last 7 days
@alex #report what feature flags are rolled out to more than half of users
```
