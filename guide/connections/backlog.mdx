...
<Steps>
  ...
  <Step title="Add Connection in CloudThinker">
    ...
    - **Domain**: your full Backlog domain, e.g. `<myorg>.backlog.com` (also `.jp` or `.backlogtool.com`) — the full host, not just the space key
...
**BACKLOG_DOMAIN** is set to just the space key instead of the full domain. Use the complete host, e.g. `<myorg>.backlog.com` (or `.jp` / `.backlogtool.com`), and reconnect.
</Accordion>
