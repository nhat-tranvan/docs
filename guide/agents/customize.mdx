...
The five default agents work out of the box — customization changes **how they talk** (tone, rules, language) and **what they can reach** (connections). When the defaults aren't enough, you can build your own agent.
...
<Note>
  Customizing agents requires the workspace **Admin** role. [Developers and Viewers](/guide/workspace-users) can chat with agents but can't edit them.
</Note>
...
<Tabs>
  <Tab title="Create with AI">
    ...
    - **Connections** — asks which to attach (cloud, database, Kubernetes, or [MCP](/guide/connections/mcp) — an open standard for plugging in external tools).
    - **Learning mode** — whether the agent saves new [skills](/guide/skills/overview) (reusable know-how it picks up from your work) automatically (**Auto**) or asks first (**Offer**).
  ...
  <Tab title="Create manually">
    ...
    <Steps>
      ...
      <Step title="Configuration">
        - **Model Tier** — pick **Light** (fast, lowest credit cost), **Pro** (balanced), or **Ultra** (max capability, highest credit cost). See [model tiers and credit cost](/guide/billing/usage#model-tiers-and-credit-cost) for current credit multipliers.
        - **Instructions** — one textarea describing how the agent should behave: what to focus on, how to respond, what to avoid. Write as plain sentences — guidelines, constraints, tone, and response format all live here. Up to 4,000 characters.
...
Change the reply language and learning mode. The agent's identity (name, @mention, role, goal) and instructions stay locked — only how it speaks and saves skills changes.
...
Connections are how an agent reaches your cloud, databases, Kubernetes, or MCP servers. Alex, Oliver, Tony, and Kai start with no connections — you decide what each can touch. Every agent has the same set of builtin tools — there is no per-agent tool toggle, so scope an agent by choosing its connections. An agent accepts up to 64 connections.
...
|-------|------------------------|
| [Alex](/guide/agents/alex) | Cloud provider accounts |
...
Good instructions cover three areas: role context (expertise for your environment), scope limits (what the agent should and shouldn't touch), and operational guidelines (procedures and escalation paths). For example:
...
- Custom and default agents alike act under your workspace's autonomy mode — Manual or Auto. See [Auto Mode](/guide/auto-mode).
