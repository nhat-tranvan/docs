...
<Info>
  Setup starts with a CloudThinker organization owner or administrator on a [messaging-eligible plan](/guide/billing/pricing#ai-features-by-plan) (Team and above). A Global Administrator or Teams Administrator completes Microsoft provider approval and Entra (Azure AD) consent separately. The same person can hold both roles, or they can coordinate the two steps.
</Info>
...
Permissions marked **Admin consent** can only be granted by a Global Administrator or Teams Administrator. During the OAuth flow, admins see a **"Consent on behalf of your organization"** checkbox:
...
In Teams, use `@CloudThinker` to start a CloudThinker thread. Reply in an existing CloudThinker thread without mentioning the bot again. To route a request to a specialist, add the agent's alias after the bot mention.
...
When an AI code review completes, CloudThinker sends a notification to your configured Teams channel as an Adaptive Card, Teams' interactive card format. The card includes:

- **Severity breakdown** — structured table showing counts by severity level (🔴 Critical, 🟠 High, 🟡 Medium, 🟢 Low)
...
<Accordion title="I'm not an admin — how do I set this up?">
  ...
  4. The bot is published to your organization's Teams app catalog
...
The team-to-workspace mapping selects the workspace a message runs in, and ordinary Teams messages run there regardless of the sender's CloudThinker role. A sender whose Teams account is not linked to a CloudThinker account runs as the workspace's owner; when no owner can be resolved, CloudThinker asks the sender to link their account first.
