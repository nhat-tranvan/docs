---
title: 'Multi-Account AWS Operations'
...
<Steps>
  ...
  <Step title="Define account-switching shortcuts">
    Open the **Shortcuts** panel (top right of any workspace) and create one [command shortcut](/guide/commands) per target account. Set the **Init Step** to instruct the agent to assume the target role ARN before your prompt runs.
...
- **[Shortcuts](/guide/commands)** — an init step injected before your prompt sets the account context without retyping the role ARN every time. Shortcuts also let you scope knowledge or instructions per account.
...
<CardGroup cols={2}>
  ...
  <Card title="Commands" icon="terminal" href="/guide/commands">
    Create shortcut commands with Init Steps for reusable prompt setups.
  </Card>
