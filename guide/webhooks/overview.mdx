---
title: Webhooks
description: "Give external systems a URL that starts an agent conversation or opens an incident in Resolve"
icon: webhook
---

Webhooks let external systems push events into CloudThinker. Each webhook gives you a unique URL; POST to it and CloudThinker starts an agent conversation or opens an incident in [Resolve](/guide/incident/overview).
...
- An **Admin** [workspace role](/guide/workspace-users) — Developers and Viewers can view webhooks and their triggers but cannot create or change them
- An external system that can send HTTPS POST requests with a JSON body

## How it works

1. You create a webhook and choose what it does: start a **Conversation** or open a **Resolve** incident.
...
sequenceDiagram
    ...
    participant Agents
    Sender->>Webhook: POST signed JSON payload
    alt Authentication fails
        Webhook-->>Sender: 401 unauthorized
    else Payload invalid
        Webhook-->>Sender: 422 unprocessable
    else Over the rate limit
        Webhook-->>Sender: 429 rate limited
    else Request accepted
        Webhook-->>Sender: 202 accepted
        Webhook->>Agents: Dispatch event
        Agents->>Agents: Start conversation or open incident
    end
```
...
| **Resolve** | Opens an incident, and optionally runs automatic RCA (root-cause analysis). `message` is optional |

For a webhook that opens an incident in Resolve, automatic RCA is **on by default** with a minimum severity of **Medium**. See [webhook integrations](/guide/incident/webhook-integrations/overview).
