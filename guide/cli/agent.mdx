...
The agent can work in two places and tells you which one it is using.
...
| **CloudThinker Sandbox** | `ct_sandbox_read` and `ct_sandbox_write` | a workspace connection credential, injected per run and never sent to your machine |
...
|---|---|---|
| `ct_sandbox_read` | A bounded read-only investigation, such as listing buckets or describing a deployment | CloudThinker Sandbox |
| `ct_sandbox_write` | One state-changing command you can name, such as a rollout restart | CloudThinker Sandbox, after the workspace decides |
...
| `/cloud` | Show whether Cloud is on, plus the workspace, connections, and mirror status |
| `/cloud on\|off` | Enable or disable cloud tools and Anna delegation for this session; existing remote work continues |
| `/where` | Show which machine runs local and cloud work |
| `/tour` | Try one local read and, when Cloud is on, one read-only cloud read |
