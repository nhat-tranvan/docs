...
The immediate `--no-wait --json` response has no `answer` field. Fetch it later with `chat status`.
...
`chat ls` and `chat status` without `--wait` exit 0 after a successful fetch. `chat status --wait` returns the run's outcome code.
