...
CloudThinker keeps one activity card for each Slack turn. The card updates in place as the agent works, so a long run stays readable instead of creating one message per tool call.

The card shows the current state, action count, and visible actions such as:
...
The activity card closes when the turn completes, fails, times out, or pauses for your approval.
