...
| **Weekly** | Refresh rules once a week, in the repository's Saturday time slot |
| **Monthly** | Refresh rules once a month, on the first Saturday |

CloudThinker gives each repository a fixed weekly time slot on a Saturday, in either the 00:00 UTC or the 12:00 UTC hour. A repository keeps the same slot, so its runs land at the same time every week. Monthly runs use that same slot time on the first Saturday of the month.

The **Convention Rules** column shows the repository's next scheduled run time.

<Info>
  The first run does not wait for the weekend. When you turn the schedule on for a repository, the first run starts soon after. The first week can therefore contain two runs: that first one, then the repository's regular Saturday slot. This is expected, and the second run only covers findings resolved since the first.
</Info>

If a run is missed, for example during maintenance, CloudThinker runs it at the next opportunity. No run is skipped.

Select **Run now** to refresh a repository's rules immediately, without waiting for the next scheduled run.
