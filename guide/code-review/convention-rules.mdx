...
CloudThinker gives each repository a fixed weekly time slot, either Saturday 00:00 UTC or Saturday 12:00 UTC. A repository keeps the same slot, so its runs land at the same time every week. Monthly runs use that same slot time on the first Saturday of the month.
