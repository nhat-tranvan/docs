...
```mermaid
flowchart LR
    A[Task starts] --> B[Eligible skills matched]
    B --> C{Relevant to this task?}
    C -->|Yes| D[Skill applied]
    C -->|No| E[Skill stays idle]
    D --> F[Agent acts]
    E --> F
```
...
| How agents use it | Applied on the surfaces the skill is attached to, when relevant to the task | Agents search the workspace knowledge available to them |
