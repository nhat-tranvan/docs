...
| `description` | Yes | A short summary of when to use the skill. Agents choose skills by reading descriptions, so a skill without one cannot be selected and will not load. |
| `argument_hint` | No | Optional hint describing the arguments the skill expects. |

## When a skill cannot load

A skill whose `SKILL.md` has no usable `description` is listed as broken. Its
detail view stays open so you can add one, but it is not offered in the `/`
command picker and the CloudThinker CLI does not install it. Add a
`description:` line to the frontmatter and save to make it loadable again.
...
<AccordionGroup>
  ...
  <Accordion title="Agent does not use the skill">
    ...
    - Agents can use eligible skills in Chat, Incident, and Assessment
    - Review loads attached skill instructions automatically
