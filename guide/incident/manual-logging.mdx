...
<Steps>
  <Step title="Open the Investigation queue">
    Go to **Resolve → Investigation** and click **Log Incident**.
  </Step>
  ...
  <Step title="Create and investigate">
    ...
    Review the context, then click **Investigate** to start RCA. Manual investigation works even when **On Duty** (automatic investigation) is off.
...
|---|---|---|
| The Incident is queued for triage | Anna gathers context and can start read-only helper agents to test hypotheses | The Incident moves to Needs your decision when RCA records a verdict or pauses |
...
| You can still correct the Incident details | The Open AI Conversation action opens the saved RCA conversation | A matching runbook may wait for approval or run if policy allows it |
...
| An alerting platform can send webhooks | Configure a [Pulse signal](/guide/incident/integrations) |
| Pulse already grouped the related Signals | Escalate the [Cluster](/guide/pulse/clusters) instead of creating a duplicate Incident |
...
<CardGroup cols={2}>
  ...
  <Card title="Resolve settings" icon="sliders" href="/guide/incident/settings">
