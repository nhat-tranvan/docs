---
title: "Resolve Integrations"
description: "Connect operational platforms, then manage how Resolve listens for Signals, reads investigation context, and sends outcomes."
icon: arrows-rotate
---

Open **Resolve → Integrations** to manage a platform as one operational relationship. Each platform groups its connection instances, the events it can receive, and the outcomes it can send back.

## Choose a layout

A control at the top switches the page between two layouts: **Split**, a platform sidebar beside a detail pane, and **Cards**, a grid of every platform. Both show the same platforms, so pick whichever reads better for you.

## Understand the three parts

| Part | Purpose | Examples |
|---|---|---|
| Connections | Authenticate the account, workspace, or project that Resolve can access. | Cloud account, Slack workspace, GitHub organization |
| Signals | Bring operational events into Pulse. | Webhook, polling, chat event |
| Outcomes | Send a response — an update, ticket, or document — to another system. | Incident update, Slack war room, Jira follow-up, draft fix merge request, postmortem |
...
Add the account, workspace, or project instances that Resolve should read. Some platforms support several connection instances. Some webhook or outcome capabilities do not require a connection.
...
A war room — a dedicated incident chat channel — has its own minimum severity: **Critical**, **High**, or **All**. Set it so routine incidents do not open a channel.

A postmortem — an after-incident summary — is written on request, not automatically. It is an outcome, not a lifecycle state, so requesting one does not change the Incident's status.
...
<CardGroup cols={2}>
  ...
  <Card title="Understand Resolve" icon="diagram-project" href="/guide/incident/overview">
    Follow an event from Pulse detection through RCA and Scoped Actions.
  </Card>
