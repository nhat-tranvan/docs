---
title: "Runbooks"
description: "Connect operational procedures, control mutating commands, and review the remediation actions Resolve executes during incidents."
icon: book-open
---

Runbooks give Resolve approved operational procedures — step-by-step fix instructions — for the Resolve stage. During RCA, an agent can find a relevant runbook, propose its commands, and apply the configured effect before any change-making command runs.

<Frame>
  <img src="/images/incidents/runbooks/02-runbook-sources-dashboard.jpg" alt="Resolve Runbooks page with connected and uploaded sources" />
</Frame>
<p style={{textAlign: 'center', fontSize: '0.9em', color: '#666', marginTop: '8px'}}>Keep the procedures that Resolve can use during an Incident in one place.</p>

## Add a runbook source

Open **Resolve → Runbooks**, then add one of these sources:
...
For a manual upload, select up to 20 Markdown files at once. Each file can be up to 5 MB. Resolve extracts mutating commands — commands that change infrastructure rather than just read it — from their code blocks so you can review the permitted effect.
...
| Effect | Behavior |
|---|---|
| Allow | Resolve can execute the command without a separate approval. |
| Require Approval | Resolve pauses and waits for a human decision. |
| Deny | Resolve cannot execute the command. |
