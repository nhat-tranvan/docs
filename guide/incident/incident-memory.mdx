...
Incident Memory is the set of saved notes Anna can write during root-cause analysis ([RCA](/guide/incident/root-cause-analysis)) and read at the start of a later investigation. It is the incident-focused part of [workspace memory](/guide/memory), and it preserves useful operational knowledge without influencing how [Pulse](/guide/pulse/overview) classifies new events.
...
Failed lessons are never presented to Anna as valid guidance. Failed recollections can remain as counter-examples so a later RCA does not repeat the same assumption.
...
- Review a failed verification before reusing the same [Scoped Action](/guide/incident/overview).
