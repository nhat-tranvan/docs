...
- **Multi-cloud structure** — a workspace can hold connections to several cloud providers at once, so one team's AWS account, GCP projects, and Azure subscription run side by side. The provider you pick at creation is the default for new work, not a limit.
...
<Steps>
  ...
  <Step title="Select a default cloud">
    Choose the provider that new connections and agent commands default to.
  </Step>
...
<Note>
  The default cloud can be changed later in workspace settings, and connecting another provider does not replace it. A workspace can hold connections on several clouds at once.
</Note>

### Provider options

Pick the default cloud; you can connect accounts on the other providers afterwards. The table lists the services agents work with per provider.
