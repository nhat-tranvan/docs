...
| プラットフォーム | サポート |
|----------|---------|
| **ClickHouse Cloud** | 全サービス |
| **セルフホスト ClickHouse** | 23.x・24.x・25.x |
| **Kubernetes 上の ClickHouse** | Altinity Operator および同等のもの |
| **Aiven for ClickHouse** | 全プラン |
| **DoubleCloud Managed ClickHouse** | 全プラン |

## 前提条件

- CloudThinker から HTTP インターフェース経由で到達可能な ClickHouse サーバー（TLS ありは `8443`、なしは `8123`）。
...
<Steps>
  ...
  <Step title="ネットワークアクセスを設定する">
    ...
    - ClickHouse Cloud の場合：サービスの IP アクセスリストに CloudThinker を追加
  ...
  <Step title="CloudThinker に接続を追加する">
    ...
    **Connect** をクリックします。CloudThinker がサーバーに到達すると、ステータスが **Connected** に変わります。
  </Step>
</Steps>
...
- ネイティブポート `9000` は HTTP インターフェースではありません。接続をそこに向けると失敗します。
</Accordion>
