...
`--no-wait --json` の即時応答には `answer` フィールドがありません。後で `chat status` から取得してください。
...
`chat ls` と `--wait` を付けない `chat status` は取得に成功すると終了コード 0 を返します。`chat status --wait` は実行結果に応じたコードを返します。
