...
`--no-wait --json`의 즉시 응답에는 `answer` 필드가 없습니다. 나중에 `chat status`로 확인하세요.
...
`chat ls`와 `--wait`가 없는 `chat status`는 조회에 성공하면 0으로 종료합니다. `chat status --wait`는 실행 결과에 따른 코드를 반환합니다.
