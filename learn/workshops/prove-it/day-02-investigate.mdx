...
Prior verdict is the layer that learns. A background agent reads the incidents people have already closed, finds the patterns that keep coming back with the same verdict, and writes them into this layer, so the list is never hand maintained and the pipeline gets quieter the longer a workspace is used.
...
The third shape is the one nobody believes until they have seen it, so here it is end to end.

| Time | What happens |
|---|---|
| 10:00 | A database fails over to its standby. It takes thirty seconds, which is what a failover is supposed to take. |
| 10:01 | Every open connection dropped at the same instant, so every client failed at the same instant and retries at the same instant. |
| 10:06 | The failover is long finished and the traffic arriving from outside is back to normal. |
| 10:30 | Still slow. |

The retries are the load now. Timeouts produce retries, retries produce load, load produces timeouts, and nothing outside the system is driving it any more. Name the three parts separately: the failover is the **trigger** and it is over, with nothing left to roll back. The **root cause** is the retry policy with no cap and no backoff, because that is the one thing whose own configuration is wrong and is still wrong right now. The missing circuit breaker is a **contributing factor**: it would have stopped this, and nobody removed it this morning.
...
| T+10 | At least two hypotheses in the log. |
| T+18 | Find the query that rules one of them out. |
| T+25 | Write the report. |

The rail is a pace, not a rule. Two is the floor, not the target. A team that reaches T+18 with one hypothesis has skipped the part that matters, and a hypothesis nobody could kill is not yet an answer.
...
```text
Give me at least two different explanations for this symptom that are both consistent with the evidence so far.
```

```text
Which single query would come out differently depending on which of those explanations is true? Run it and show me the raw output.
```
...
| **02 · At least two hypotheses, one ruled out** | The evidence for each, and what ruled the loser out. |
...
| Minutes | Show |
|---|---|
| 3 | **Your mechanism and evidence.** Which hypotheses you held, which query ruled one out, and what the evidence was. |
