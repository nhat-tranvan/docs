...
One team, one organization. Whoever registered invites the rest, so everyone works in the same workspace, sees the same demo connection and writes one report together.

<Steps>
  <Step title="Open organization settings">
    Go to [Admin Settings · Organization](https://app.cloudthinker.io/admin-settings/organization) and scroll to the Members section.
  </Step>

  <Step title="Send the invites">
    Click **Invite Members**, add your teammates' email addresses (up to 10 at a time), choose a role and click **Send Invites**. **Developer** is the right role for today: it runs agents in the assigned workspace and touches nothing in billing or organization settings.
  </Step>
</Steps>

<Note>
Only the Owner or an Admin can invite. Teammates accept from the invitation email and land in the same organization. Roles and workspace access in full: [Organizations](/guide/organization).
</Note>
