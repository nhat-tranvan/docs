...
<Steps>
  <Step title="Tạo CloudThinker Webhook">
    1. Trong CloudThinker, vào **Resolve** → **Settings** → **Integrations**
    ...
    | Service | `detail.configuration.metrics[0].metricStat.metric.namespace` | Namespace dịch vụ AWS (ví dụ: `AWS/EC2`) |
...
<Accordion title="Không có incident nào được tạo">
  ...
  3. **Kiểm tra log CloudThinker** — Vào Resolve → Settings → Integrations → chọn webhook → tab Logs để xem lịch sử gửi
