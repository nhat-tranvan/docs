...
CloudKeepers là các monitor tự trị thực thi các biện pháp bảo vệ về chi phí, bảo mật, và hiệu suất trên tất cả tài khoản đám mây và Kubernetes cluster đã kết nối.

<Note>Bề mặt cũ này không còn trong thanh điều hướng hiện tại. Route `/resources/cloudkeepers` vẫn hoạt động trong thời gian ngừng sử dụng. Dùng [Optimize Findings](/vi/guide/cost-optimization/recommendations) cho quy trình phát hiện chi phí hiện tại.</Note>
...
<Steps>
  <Step title="Mở Keepers">
    Mở trực tiếp route cũ `/resources/cloudkeepers`. Giao diện hướng dẫn ba bước: kết nối tài khoản cloud, bật keepers và chạy lần quét đầu tiên. Nhấp **Enable Your First Keepers** để bắt đầu.
